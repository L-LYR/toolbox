...
using BoundedSPSC = toolbox::container::BoundedSPSCQueue<T, Size>;
...
TEST(BoundedSPSCQueue, SPSCCorrectnessTest) {
  BoundedSPSC<uint64_t, 1024> iq;
  RunSPSCCorrectnessTest(iq, 1 << 20);
  BoundedSPSC<std::string, 1024> sq;
...
TEST(BoundedSPSCQueue, DtorTest) {
  {
    BoundedSPSC<DtorCounter, 1024> q;
    for (int i = 0; i < 10; ++i) {
